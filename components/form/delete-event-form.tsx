  ...
  return (
    <form
      action={async (data: FormData) =>
        ...
        deleteEvent(data, id, "delete").then((res) => {
          if (res.error) {
            toast.error(res.error);
          } else {
            va.track("Deleted Event");
            router.refresh();
    ...
    >
      <div className="relative flex flex-col space-y-4 p-5 sm:p-10">
        <h2 className="font-cal text-xl dark:text-white">Delete event</h2>
        <p className="text-sm text-stone-500 dark:text-stone-400">
          Deletes your event permanently. Type in the name of your event{" "}
          <b>{eventName}</b> to confirm.
        </p>
