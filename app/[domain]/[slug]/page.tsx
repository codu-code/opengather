...
import { getEventData, getCommunityData } from "@/lib/fetchers";
...
export async function generateMetadata({
  ...
  return {
    ...
    //     alternates: {
    //       canonical: `https://${communityData.customDomain}/${params.slug}`,
    //     },
    //   }),
  };
}
...
export async function generateStaticParams() {
  ...
  const allPaths = allEvents
    .flatMap(({ community, slug }) => [
      community?.subdomain && {
        domain: `${community.subdomain}.${process.env.NEXT_PUBLIC_ROOT_DOMAIN}`,
        slug,
      },
      community?.customDomain && {
        domain: community.customDomain,
        slug,
      },
    ])
    .filter(Boolean);
    ...
    <>
      <div className="flex flex-col items-center justify-center">
        ...
        <a
          ...
          href={
            data.community?.user?.username
              ? `https://twitter.com/${data.community.user.username}`
              : `https://github.com/${data.community?.user?.gh_username}`
          }
          rel="noreferrer"
          target="_blank"
        >
          <div className="my-8">
            <div className="relative inline-block h-8 w-8 overflow-hidden rounded-full align-middle md:h-12 md:w-12">
              {data.community?.user?.image ? (
                <BlurImage
                  alt={data.community?.user?.name ?? "User Avatar"}
                  height={80}
                  src={data.community.user.image}
                  width={80}
                />
              ) : (
            ...
            <div className="text-md ml-3 inline-block align-middle dark:text-white md:text-lg">
              by <span className="font-semibold">{data.community?.user?.name}</span>
            </div>
          </div>
        </a>
      </div>
